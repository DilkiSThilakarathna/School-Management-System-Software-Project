...
exports.getProfile = async (req, res, next) => {
    ...
    const parents = await zeroParamPromise('SELECT * FROM parent');
...
exports.getAddStudent = async (req, res, next) => {
    ...
    const sql1 = 'SELECT * from parent';
    const results = await zeroParamPromise(sql);
    const results1 = await zeroParamPromise(sql1);
    let departments = [];
    let parents = [];
    for (let i = 0; i < results.length; ++i) {
        departments.push(results[i].dept_id);
    ...
    for (let i = 0; i < results1.length; ++i) {
        parents.push(results1[i].p_name);
    }
    console.log(parents);
...
exports.postAddStudent = async (req, res, next) => {
    ...
    const sql3 = 'select p_id from parent where p_name=?';
    const results1 = await queryParamPromise(sql3, [parent]);
    console.log(results1[0].p_id, parent);
    let section = 1;
...
exports.getReport = async (req, res, next) => {
    ...
    res.render('Admin/Report/setReport', {
        files,
        page_name: 'reports',
    });
};
...
// 4.2 Add Report
exports.getAddReport = async (req, res, next) => {
    ...
    res.render('Admin/Report/addReport', {
        page_name: 'reports',
        students: students,
    });
};
...
exports.postAddReport = async (req, res, next) => {
    ...
    await ejs.renderFile(path.join(__dirname,'..', '/views/Admin/Report', "getReport.ejs"), {student:student, students:st_marks,page_name:'reports'}, (err, data) => {
        if (err) {
            console.log(err);
        } else {
            console.log(st_marks);
